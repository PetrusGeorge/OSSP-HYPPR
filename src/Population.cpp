...
Population::Population(Parameters *parameters) : parameters(parameters) {
    ...
    for (int i = 0; i < parameters->numJobs; i++) {
        elementos_iniciais.push_back(i);
    }
    ...
    for (unsigned int i = 0; i < parameters->populationSize; i++) {
        ...
        // if (i <= parameters->populationSize - 6) {
        //     randomIndiv = new Individual(parameters);
        // } else {
        //     int e = rand() % elementos_iniciais.size();
        //     randomIndiv = new Individual(parameters, elementos_iniciais[e]);
        //     elementos_iniciais.erase(elementos_iniciais.begin() + e);
        // }
        ...
        //     cout << randomIndiv->chromosome[j] << " ";
        // }
        // cout << "        " << randomIndiv->solutionCost.evaluation << endl;
        // randomIndiv->localSearch = new LocalSearch(parameters, randomIndiv);
        education(randomIndiv);
        addIndividual(randomIndiv);
        //         double nd = cpuTime() - s;
        // cout << randomIndiv->solutionCost.evaluation << " " << nd << endl;
                //         for (int j = 0; j < randomIndiv->chromosome.size(); j++) {
                //     cout << randomIndiv->chromosome[j] << " ";
                // }
                // cout << endl << endl;
        delete randomIndiv;
    ...
    // getchar();
    // this->tempo = this->tempo + cpuTime() - startTime;
    // cout << fim;
    // getchar();
    // cout << endl;
    // getchar();
...
void Population::diversify() {
    ...
    // cout << subPopulation->numberIndividuals;
    // getchar();
    
    // for (int i = parameters->numberElite; i < subPopulation->numberIndividuals; i++) {
    //     subPopulation->individuals[i]->random_swap();
    // }
    ...
    for (unsigned int i = 0; i < parameters->populationSize; i++) {
        ...
        if (!elementos_iniciais.empty()) {
            int e = rand() % elementos_iniciais.size();
            randomIndiv = new Individual(parameters, elementos_iniciais[e]);
            elementos_iniciais.erase(elementos_iniciais.begin() + e);
        } else {
            int e = rand() % parameters->numJobs;
            randomIndiv = new Individual(parameters, e);
        }
...
void Population::education(Individual *indiv) {
    ...
    //indiv->localSearch->runSearchTotal();
}